...
GREENMAIL_IMAP_SECURE="${GREENMAIL_IMAP_SECURE:-false}"
...
if [[ "$GREENMAIL_IMAP_SECURE" == "true" ]]; then
  GREENMAIL_CONTAINER_IMAP_PORT=3993
fi
...
if [[ "$EXTERNAL_ENDPOINT" -eq 1 ]]; then
  ...
  docker run -d --rm --name "$NAME" \
    ...
    -p "$GREENMAIL_IMAP_PORT:${GREENMAIL_CONTAINER_IMAP_PORT}" \
    "$IMAGE"
...
export MAIL_IMAP_DEFAULT_SECURE="$GREENMAIL_IMAP_SECURE"
